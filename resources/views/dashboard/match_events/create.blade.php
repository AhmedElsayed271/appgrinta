    ...
    <div class="card card-custom">
        <div class="card-header flex-wrap border-0 pt-6 pb-0">
            <div class="card-title">
                <h3 class="card-label">{{__('site.matchEvent.create')}}
        ...
        <div class="card-body">
            <form class="form" method="post" action="{{route('dashboard.matches.events.store',$match->id)}}" enctype="multipart/form-data">
                @csrf
                <div class="card-body">
                    <div class="form-group row">
                        <label for="minute" class="col-form-label col-lg-3 col-sm-12">{{__('site.matchEvent.minute')}}</label>
                        <div class="col-lg-4 col-md-9 col-sm-12">
                            <input type="text" name="minute" class="form-control @error('minute') is-invalid @enderror" id="minute" value="{{old('minute')}}" >
                            @error('minute')
                    ...
                    <div class="row">
                                ...
                                <div class=" col-lg-9 col-md-9 col-sm-12">
                                    <select class="form-control select2" id="team_id" name="team_id">
                                        ...
                                        @foreach($teams as $team)
                                            <option value="{{$team->id}}" data-image="{{$team->image_path}}">{{$team->name}}</option>
                                        @endforeach
                                    </select>
                                    @error('team_id')
                                    <div class="invalid-feedback" role="alert">
                        ...
                        <div class="col-md-6">
                            <div class="form-group row">
                                <label for="player_enter" class="col-form-label col-lg-3 col-sm-12">{{__('site.player.enter')}}</label>
                                <div class=" col-lg-9 col-md-9 col-sm-12">
                                    <select class="form-control select2" id="player_enter" name="player_enter">
                                        <option value=""></option>
                                    </select>
                                    @error('player_enter')
                                    <div class="invalid-feedback" role="alert">
                    ...
                    <div class="form-group row">
                        ...
                        <div class="col-lg-4 col-md-9 col-sm-12">
                            <input type="text" name="player_name" class="form-control @error('player_name') is-invalid @enderror" id="player_name" value="{{old('player_name')}}" >
                            @error('player_name')
                            <div class="invalid-feedback" role="alert">
                                <strong>{{ $message }}</strong>
                            </div>
...
@section('scripts')
    ...
    <script !src="">
        ...
        var KTSelect2 = function() {
            ...
            var demos = function() {
                // basic
                $('#player_id,[name=status],#player_enter').select2({
        ...
        function players(team){
            ...
            $.ajax({
                ...
                success:function (data){
                    $.each(data,function (key,value){
                        players += `<option value="${value.id}" data-image="${value.image_path}">${value.first_name + ' ' + value.last_name}</option>`
                    })
                    $('#player_id,#player_enter').html(`${players}`);
                }
            })
        }
    </script>
